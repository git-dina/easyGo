...
namespace EasyGo.Classes.ApiClasses
{
    public class PurchaseInvoice
    {
       ...
       public List<CashTransfer> ListPayments { get; set; }
        ...
        public async Task<PurchaseInvoice> GetInvoiceToReturn(long invoiceId)
        {
           PurchaseInvoice items = new PurchaseInvoice();
            Dictionary<string, string> parameters = new Dictionary<string, string>();
            parameters.Add("invoiceId", invoiceId.ToString());
            ...
            IEnumerable<Claim> claims = await APIResult.getList("PurchaseInvoice/GetInvoiceToReturn", parameters);
            foreach (Claim c in claims)
            {
                if (c.Type == "scopes")
                {
